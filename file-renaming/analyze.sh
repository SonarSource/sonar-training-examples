...
sonar-scanner -Dsonar.host.url=$SQ_URL -Dsonar.login=$SQ_TOKEN -Dsonar.projectVersion=1.0 

# Rename package, rename and move file
./rename.sh

# Scan again, change version to create a leak
sonar-scanner  -Dsonar.host.url=$SQ_URL -Dsonar.login=$SQ_TOKEN -Dsonar.projectVersion=2.0 
